...
def build_features(df: pd.DataFrame, capacity_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    year = df["hourly"].dt.year
    capacity_matched = capacity_df.set_index(capacity_df["time_stamp"].dt.year).loc[year].reset_index(drop=True)
    ...
    targets = pd.concat([(df["solar_generation"] / capacity_matched['solar_generation_capacity']).rename("solar_generation"), 
                        ...
                        (capacity_matched['solar_generation_capacity'] + capacity_matched['wind_generation_on_capacity'] + capacity_matched['wind_generation_off_capacity'])).rename("renewable_total_mw")], axis=1)
